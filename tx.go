...
// FindLE retrieves key and value that is less than or equal to the input key.
func (t *Tx) FindLE(ctx context.Context, pkey string) (string, string, error) {
	kvs := t.allLive(true /* sort */)
	i := sort.Search(len(kvs), func(i int) bool {
		return kvs[i][0] >= pkey
	})
	found := i < len(kvs) && kvs[i][0] == pkey
	if !found && i == 0 {
		return "", "", os.ErrNotExist
	}
	var k, v string
	if found {
		...
		k, v = kvs[i-1][0], kvs[i-1][1]
	}
	t.touch(k)
	return k, v, nil
}

// FindLT retrieves key and value that is strictly less than the input key.
func (t *Tx) FindLT(ctx context.Context, pkey string) (string, string, error) {
	kvs := t.allLive(true /* sort */)
	i := sort.Search(len(kvs), func(i int) bool {
		return kvs[i][0] >= pkey
	})
	...
	t.touch(kvs[i-1][0])
	return kvs[i-1][0], kvs[i-1][1], nil
}

// FindGE retrieves key and value that is greater than or equal to the input
// key.
func (t *Tx) FindGE(ctx context.Context, pkey string) (string, string, error) {
	kvs := t.allLive(true /* sort */)
	i := sort.Search(len(kvs), func(i int) bool {
		return kvs[i][0] >= pkey
	})
	...
	t.touch(kvs[i][0])
	return kvs[i][0], kvs[i][1], nil
}

// FindGT retrieves key and value that is strictly greater than to the input
// key.
func (t *Tx) FindGT(ctx context.Context, pkey string) (string, string, error) {
	kvs := t.allLive(true /* sort */)
	i := sort.Search(len(kvs), func(i int) bool {
		return kvs[i][0] >= pkey
	})
